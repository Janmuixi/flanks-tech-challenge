...
What to deliver
1. Ingest & normalize
An endpoint that reads the CSV and persists records to a database using a schema you design and justify. Parse the CSV directly — don't rely on libraries that silently swallow malformed rows.

2. REST API with the following endpoints:
...
6. README
Explain:
How to run the application and consume the API
...
Constraints
Language: Python or TypeScript (Depending on the process)
Time
